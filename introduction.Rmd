...
first lets sample some Gumbel distributions.
...
```{r}
 one_hot %>% mutate(OH = (OH*10)%/%1) %>%filter(dimension %in% c(1,10))%>% plot_ly(x=~OH, type="histogram",color=~dimension,bingroup=1)%>%
  layout(
  barmode="stack",
  bargap=0.1,yaxis = list(range = c(0, 1000)))
