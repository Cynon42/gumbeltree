...
```
not working properly if you ask me. lets check a synthetic model again. this time with 2 layers.
...
```
Found the bug. When y is passed to BRMS, it is re-ordered from max to min. This is the opposite order that I calculated, so the model would be randomising y essentially.
...
I found the main bug. the for loop (j in L_T:1) was never entered for trees larger than L_T=1 because the algorithm only accepts increasing j!!
```{r}
...
Awesome. Another bug. All the mu_X are the same number. try to fix
...
```
Lets test a 2 dimensional f(x) and a 4 level tree.
...
X = X%>%mutate(p01 = quantiles[,1],p50 = quantiles[,2],p99 = quantiles[,3])
X%>%plot_ly(x=~x2, y=~x3, z=~y, type = "scatter3d", mode = "markers",size =1, name = "y", color = "blue")%>%
add_trace(x=~x2, y=~x3, z=~p01, type = "scatter3d", mode = "markers",size =0.5,name="predicted at 1%", color = "green")%>%
  add_trace(x=~x2, y=~x3, z=~p99, type = "scatter3d", mode = "markers",size =0.5,name="predicted at 99%", color = "orange")
